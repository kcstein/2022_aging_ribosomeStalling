...
### CAG analysis
### Align ribosome occupancy at start of polybasic stretch ###
CAG <- as.data.table(read.csv("/Users/KevinStein/Desktop/Lab/Bioinformatics/ReferenceData/Worm/PolybasicRegions/CAG3.csv", header = FALSE, stringsAsFactors = TRUE))
colnames(CAG) <- c("orf", "CAGposition")
CAG[, name := paste0(orf, "_", CAGposition)]

CAG_dt <- N2_dtA[CAG, allow.cartesian=TRUE]
CAG_dt[, adjusted := position - CAGposition]

### Normalize across polybasic interval ###
test <- CAG_dt[adjusted >= -40 & adjusted <= 40]
setkeyv(test, c("name"))
setkeyv(CAG_dt, c("name"))
test[, D1_1A_rpc_adjusted := mean(D1_1A), by = name]
test[, D1_2A_rpc_adjusted := mean(D1_2A), by = name]
test[, D12_1A_rpc_adjusted := mean(D12_1A), by = name]
test[, D12_2A_rpc_adjusted := mean(D12_2A), by = name]
test <- test[, .SD[which.min(position)], by = name]
i <- cbind(match(CAG_dt$name, test$name))
CAG_dt <- cbind(CAG_dt, D1_1A_rpc_adjusted = test[i]$D1_1A_rpc_adjusted)
CAG_dt <- cbind(CAG_dt, D1_2A_rpc_adjusted = test[i]$D1_2A_rpc_adjusted)
CAG_dt <- cbind(CAG_dt, D12_1A_rpc_adjusted = test[i]$D12_1A_rpc_adjusted)
CAG_dt <- cbind(CAG_dt, D12_2A_rpc_adjusted = test[i]$D12_2A_rpc_adjusted)
CAG_dt[, D1_1A_norm := D1_1A / D1_1A_rpc_adjusted]
CAG_dt[, D1_2A_norm := D1_2A / D1_2A_rpc_adjusted]
CAG_dt[, D12_1A_norm := D12_1A / D12_1A_rpc_adjusted]
CAG_dt[, D12_2A_norm := D12_2A / D12_2A_rpc_adjusted]
# saveRDS(CAG_dt, "CAG_dt.rds")


### Plot ribosome occupancy around site of interest ###
CAG_dt <- readRDS("doc/CAG_dt.rds")

ggplot(data = CAG_dt[D1_1A_rpc_adjusted >= 0.1 & D1_2A_rpc_adjusted >= 0.1 & 
                       D12_1A_rpc_adjusted >= 0.1 & D12_2A_rpc_adjusted >= 0.1,
                     .(adjusted, D1 = movingAverage(((D1_1A_norm + D1_2A_norm) / 2), n=1, center=T),
                       D12 = movingAverage(((D12_1A_norm + D12_2A_norm) / 2), n=1, center=T))]) + xlim(-25, 25) +
  stat_summary(aes(adjusted, D1), fun.y = "mean", geom = "line", size=0.5, color = 'black') +  
