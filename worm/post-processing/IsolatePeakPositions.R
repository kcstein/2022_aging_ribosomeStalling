...
# Fishers, replicates separately
stalling_peaks_all_replicates[, odds_combined := (odds1 + odds2) / 2]
orfs <- stalling_peaks_all_replicates[, .SD[which.min(position)], by = orf]
peaks_subset <- NULL 
odds_subset <- NULL
final_peaks <- NULL
final_odds <- NULL
...
gene <- NULL
for (g in orfs$orf) {
  odds <- stalling_peaks_all_replicates[g]$odds_combined
  peaks <- stalling_peaks_all_replicates[g]$position
  while (length(peaks) > 0) {
    y=peaks[1]
    while (y %in% peaks) {
      peaks_subset1 <- y
...
stalling_peaks_replicates1 <- data.table(orf = gene, peak_start = peak_start, peak_end = peak_end,
                             peak = final_peaks,
                             odds = final_odds)
stalling_peaks_replicates1[, ID := as.character(base::paste(orf, peak, sep = "_"))]
setkeyv(stalling_peaks_replicates1, c("orf"))
stalling_peaks_replicates <- stalling_peaks_all_replicates[stalling_peaks_all_replicates$ID %in% stalling_peaks_replicates1$ID]
i <- cbind(match(stalling_peaks_replicates$ID, stalling_peaks_replicates1$ID))
stalling_peaks_replicates <- cbind(stalling_peaks_replicates, peak = stalling_peaks_replicates1[i]$peak)
stalling_peaks_replicates <- cbind(stalling_peaks_replicates, peak_start = stalling_peaks_replicates1[i]$peak_start)
stalling_peaks_replicates <- cbind(stalling_peaks_replicates, peak_end = stalling_peaks_replicates1[i]$peak_end)
saveRDS(stalling_peaks_replicates, "stalling_peaks_replicates.rds")
write.csv(stalling_peaks_replicates, "stalling_peaks_replicates.csv")
# stalling_peaks_replicates_max <- stalling_peaks_replicates[, .SD[which.max(odds_combined)], by = orf]
# saveRDS(stalling_peaks_replicates_max, "stalling_peaks_replicates_max.rds")
# write.csv(stalling_peaks_replicates_max, "stalling_peaks_replicates_max.csv")
